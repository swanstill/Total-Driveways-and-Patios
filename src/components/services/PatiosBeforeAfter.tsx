...
export default function PatiosBeforeAfter() {
    ...
    <section className="py-[120px]" style={{ background: "var(--svc-section)" }}>
      <div className="max-w-[1280px] mx-auto px-6">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} className="text-center max-w-[680px] mx-auto mb-16">
          ...
          <motion.h2 custom={1} variants={{ hidden: { opacity: 0, y: 20 }, visible: (i) => ({ opacity: 1, y: 0, transition: { duration: 0.5, delay: i * 0.1 } }) }} className="mt-5 mb-5" style={{ color: "var(--svc-heading)", fontFamily: "var(--font-manrope), sans-serif", fontSize: "clamp(28px, 2.8vw, 42px)", fontWeight: 700, letterSpacing: "-0.02em" }}>
            See the Difference We Make
          </motion.h2>
          <motion.p custom={2} variants={{ hidden: { opacity: 0, y: 20 }, visible: (i) => ({ opacity: 1, y: 0, transition: { duration: 0.5, delay: i * 0.1 } }) }} className="text-[17px]" style={{ color: "var(--svc-body)" }}>
            Drag the slider to compare the before and after of a recent patio project in Barnsley.
          </motion.p>
        </motion.div>

        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={{ hidden: { opacity: 0, y: 30 }, visible: { opacity: 1, y: 0, transition: { duration: 0.6 } } }} className="max-w-[900px] mx-auto select-none">
          <div
            ref={containerRef}
          ...
          >
            <div className="absolute inset-0" style={{ background: "url('/patios-after.jpg') center/cover no-repeat" }} />
            <div className="absolute inset-0" style={{ background: "url('/patios-before.jpg') center/cover no-repeat", clipPath: `inset(0 ${100 - sliderPos}% 0 0)` }} />
            ...
            <div className="absolute bottom-4 right-4 z-10 text-[11px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-[8px]" style={{ background: "rgba(8,33,63,0.8)", color: "rgba(255,255,255,0.85)" }}>After</div>
          </div>
        </motion.div>
      </div>
    </section>
